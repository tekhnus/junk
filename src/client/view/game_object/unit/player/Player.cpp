...
Player::Player()
    : body(1), inner(12 / 20.0), gun(sf::Vector2f(25.0 / 20.0, 4.0 / 20.0)) {
  ...
  healthBarBackground.setOrigin(
      sf::Vector2f(body.getRadius(), 8 / 20.0f + body.getRadius()));
...
void Player::updatePlayer(const model::Player& player) {
  ...
  healthBar.setSize(
      sf::Vector2f(body.getRadius() * 2 * healthScale, 4.0f / 20.0));
